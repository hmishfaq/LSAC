...
class Action_Distribution:
    ...
    def get_act_dist(self, logits):
        ...
        if has_act_lim:
            act_dist.act_high_lim = getattr(self, "act_high_lim").to(logits.device)
            act_dist.act_low_lim = getattr(self, "act_low_lim").to(logits.device)
...
class TanhGaussDistribution:
    def __init__(self, logits):
        ...
        self.act_high_lim = torch.tensor([1.0], device='cuda:0')
        self.act_low_lim = torch.tensor([-1.0], device='cuda:0')
