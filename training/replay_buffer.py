...
__all__ = ["ReplayBuffer", "DiffusionBuffer"]
...
class DiffusionBuffer:
    """Implementation of synthetic buffer to store best state action pairs.
    NOTE: self.buf["best_act"] stores unnormalized actions, but action gradient operates on normalized actions
    """
    ...
    def __init__(self, index=0, **kwargs):
        ...
        self.buf = {
            "obs": np.zeros(combined_shape(self.max_size, self.obsv_dim), dtype=np.float32),
            "obs2": np.zeros(combined_shape(self.max_size, self.obsv_dim), dtype=np.float32),
            "best_act": np.zeros(combined_shape(self.max_size, self.act_dim), dtype=np.float32),
            "rew": np.zeros(self.max_size, dtype=np.float32),
            "done": np.zeros(self.max_size, dtype=np.float32),
        }        
        self.ptr, self.size, = (0, 0,)
    ...
    def store(
        ...
        self.buf["obs"][self.ptr] = obs
        self.buf["obs2"][self.ptr] = next_obs
        self.buf["best_act"][self.ptr] = act
        self.buf["rew"][self.ptr] = rew
        self.buf["done"][self.ptr] = done
        self.ptr = (self.ptr + 1) % self.max_size
        self.size = min(self.size + 1, self.max_size)
    ...
    def replace_actions(self, best_actions, idxs):
        np.copyto(self.buf["best_act"][idxs], best_actions)