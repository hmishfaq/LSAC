"""Main diffusion generator class, adapted from:
...
class DiffuserTrainer(Trainer):
    def __init__(
            self,
            diffusion_model,
            train_batch_size: int = 256,        # NOTE: in offline cfg this is 1024
            gradient_accumulate_every: int = 1,
            train_lr: float = 3e-4,
            lr_scheduler: Optional[str] = 'cosine',
            train_num_steps: int = 100000,        # NOTE: originally 100000
            ema_update_every: int = 10,
            ema_decay: float = 0.995,
            adam_betas: Tuple[float, float] = (0.9, 0.99),
            save_and_sample_every: int = 10000,
    ...
    def train_from_replay_buffer(self, buffer: ReplayBuffer, num_steps: Optional[int] = None):
        num_steps = num_steps or self.train_num_steps
        for j in range(num_steps):
            b, _ = buffer.sample_batch(self.batch_size)
            obs = b['obs']
            next_obs = b['obs2']
            actions = b['act']
            rewards = b['rew'][:, None]
            ...
            if j % 1000 == 0:
                print(f'[{j}/{num_steps}] diffuser loss: {loss:.4f}')
    ...
    def make_inputs_from_replay_buffer(
        ...
        ptr_location = buffer.ptr
        obs = buffer.buf["obs"][:ptr_location]
        actions = buffer.buf["act"][:ptr_location]
        next_obs = buffer.buf["obs2"][:ptr_location]
        rewards = buffer.buf["rew"][:ptr_location]
        inputs = [obs, actions, rewards[:, None], next_obs]
        if model_terminals:
            terminals = buffer.buf["done"][:ptr_location].astype(np.float32)
            inputs.append(terminals[:, None])
        return np.concatenate(inputs, axis=1)

    def update_normalizer(self, buffer: ReplayBuffer, device='cpu'):
        data = self.make_inputs_from_replay_buffer(buffer, self.model_terminals)
        data = torch.from_numpy(data).float()
