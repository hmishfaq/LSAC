...
X_FACTOR = 1e6
colour_list = {
    "DMLMC (ours)": (254 / 255, 6 / 255, 6 / 255),
    "DSAC-T": (168 / 255, 47 / 255, 47 / 255),
...
environments = [
    "all_data_ant.txt",
    "all_data_halfcheetah.txt",
    "all_data_hopper.txt",
    "all_data_humanoid.txt",
    "all_data_swimmer.txt",
    "all_data_walker2d.txt"
]
...
for file in environments:
    ...
    for algo in algorithms:
        ...
        if algo == 'OURS':
            ax.plot(steps, rew_mean, label="DMLMC (ours)", linewidth=2, color=colour_list["DMLMC (ours)"] + (1,))  # alpha=0.75)
            ...
            ax.plot(steps, rew_mean, label="DSAC-T", linewidth=2,
                    color=colour_list["DSAC-T"] + (1,))  # alpha=0.75)
    ...
    plt.savefig(f"{env_id}_plot.pdf", bbox_inches="tight")
    plt.show()
